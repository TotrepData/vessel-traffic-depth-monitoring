...
3. El pipeline actualizará Bronze → Silver → Gold automáticamente.
...
## 14.1. Limitaciones de Acceso a S3 y Solución Implementada

CloudLabs (cluster compartido) no permite acceder a S3 directamente desde Spark, incluso si los buckets son públicos.  
...
Para resolverlo, se descargaron los datos NOAA localmente usando AWS CLI y luego se subieron a un volumen administrado en Databricks.

### Fase 0 – Instalar AWS CLI
...
### Fase 1 – Descargar datos desde S3

```
aws s3 cp s3://noaa-dcdb-bathymetry-pds/csb/csv/2025/01/01/ ~/noaa_data/ --recursive --no-sign-request
```
...
1. Crear schema en UC  
2. Crear volumen managed  
3. Subir los CSV usando “Upload files to volume”

### Resultado Final

Los archivos NOAA se leen localmente desde:

```
/Volumes/labs_56754_cs713b/vessel_traffic_monitoring/noaa_raw_data/
```
...
## 14.2. H3 – Indexación Geográfica Hexagonal

H3 divide el mundo en celdas hexagonales de tamaño uniforme (≈1200 km² en resolución 5).

### ¿Por qué es crítico?

Las coordenadas AIS y NOAA nunca coinciden exactamente.  
Sin H3, no sería posible cruzar profundidad con posiciones AIS.

Con H3:

- AIS → se asigna a una celda hexagonal  
- NOAA → se asigna a la misma grilla  
- JOIN rápido y geoespacialmente coherente  

### Estrategia aplicada

1. Columna H3 en AIS y NOAA  
2. Ventanas de tiempo de 30 minutos  
3. Agregación de profundidad por celda  
4. JOIN AIS–NOAA por:
   - h3_cell
   - ventana temporal superpuesta

### Resultado

Cada registro AIS queda enriquecido con:
- profundidad promedio cercana  
- min/max profundidad  
- muestras disponibles  
- celda geográfica asociada  

Ejemplo:

```
“El barco X en tiempo Z, navegaba en la celda H3 Y,
con una profundidad promedio de W metros.”
```

---

# 15. Databricks Workflow Job (Ejecución del Pipeline)

## 15.1. Objetivo del Workflow

El Workflow automatiza la ejecución completa del pipeline, garantizando que:

- Bronze procese nuevos archivos AIS.  
- Silver procese solo días faltantes.  
- Gold agregue solo días nuevos para dashboards.  

Es la forma recomendada de ejecutar el proyecto de punta a punta.

---

## 15.2. Estructura del Workflow

El Workflow está compuesto por tres tareas secuenciales:

```
Task 1 → 01_bronze.py
Task 2 → 02_silver.py (depende de Bronze)
Task 3 → 03_gold.py   (depende de Silver)
```

Cada tarea utiliza el mismo cluster y comparte la configuración de `00_config.py`.

---

## 15.3. Lógica General de Ejecución

### ● **Bronze**
- Identifica archivos AIS nuevos.  
- Procesa N por corrida (configurable).  
- NOAA se recarga completo.

### ● **Silver**
- Identifica días faltantes comparando Bronze vs Silver.  
- Procesa únicamente esos días.  
- Enriquecimiento AIS–NOAA usando H3.

### ● **Gold**
- Identifica días faltantes comparando Silver vs Gold.  
- Agrega métricas agregadas por día y celda.

Esto asegura alta eficiencia:  
**nada se reprocesa innecesariamente**.

---

## 15.4. Configuración del Cluster

Recomendación:

### ● **Development**
- Existing Cluster  
- Auto-termination activado  
- Library: `h3` instalada en cluster
...
## 15.5. Programación del Workflow

El Workflow puede ejecutarse:

- Manualmente (útil en pruebas)  
...
## 15.6. Logs, Errores y Trazabilidad

Databricks permite ver:

- Logs por tarea  
- Logs por cluster  
- Tiempos por fase  
- Fallas detalladas con stacktrace  

Esto facilita depuración y control del pipeline.

---

## 15.7. Reproducibilidad

Todo el pipeline es reproducible porque:

- Las rutas están centralizadas en `00_config.py`.  
- Bronze, Silver y Gold son idempotentes por diseño.  
